...
export function registerColumnTools(server: McpServer, store: DataStore) {
  server.tool(
    'add_column',
    ...
    async ({ tableId, name, type, required, min, max, values, configRef }) => {
      try {
        ...
        const schema = await store.addColumn(tableId, field as any)
  ...
  server.tool(
    ...
    async ({ tableId, columnName, name, type, required, min, max, values }) => {
      try {
        ...
        const schema = await store.updateColumn(tableId, columnName, updates)
        return { content: [{ type: 'text' as const, text: JSON.stringify(schema, null, 2) }] }
      } catch (e) {
        return { content: [{ type: 'text' as const, text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true }
      }
  ...
  server.tool(
    ...
    async ({ tableId, columnName }) => {
      try {
        const schema = await store.removeColumn(tableId, columnName)
        return { content: [{ type: 'text' as const, text: JSON.stringify(schema, null, 2) }] }
      } catch (e) {
        return { content: [{ type: 'text' as const, text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true }
      }
